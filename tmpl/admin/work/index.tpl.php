                            ...
                            <div class="box box-solid">
                                ...
                                <div class="box-body no-padding">
                                    <ul class="nav nav-pills nav-stacked">
                                        <li class="active"><a href="/admin.php?module=work&action=index"><i class="fa fa-inbox"></i> 待处理 <span class="label label-primary pull-right">12</span></a></li>
                                        <li><a href="/admin.php?module=work&action=finish"><i class="fa fa-envelope-o"></i> 已处理</a></li>
...
<td>￥:{{value['inApplyMoney']}}元</td>
<td>￥:{{value['inBearMoney']}}元</td>
<td>{{value['strLoanType']}}</td>
<td>{{value['strTitle']}}</td>
<td>{{value['strSalesman']}}</td>
<td>{{value['tCreateTime']}}</td>
<td><button class="btn btn-block btn-primary" onclick="openUrl('{{value['strWorkNum']}}','{{value['strUserId']}}');">办理</button></td>
</tr>
{{/each}}
</script>
<script>
    ...
    $.post("/admin.php?module=ajax&action=workList",
            function(listData) {
                var data = {
                ...
                var html = template('work', data);
                $("#work-list").html(html);
            }, "json");
            
    function openUrl(strWorkNum,strUserId){
        var index = layer.open({
            type: 2,
            content: '/admin.php?module=work&action=handle&strWorkNum='+strWorkNum+"&strUserId="+strUserId,
            area: ['320px', '195px'],
            maxmin: true
        });
        layer.full(index);
